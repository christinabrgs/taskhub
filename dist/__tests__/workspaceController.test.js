...
describe("Workspace Controller", () => {
    ...
    describe("POST /workspaces/:wsId/apikeys", () => {
        ...
        it("should return 404 for non-existent workspace", async () => {
            ...
            const response = await (0, supertest_1.default)(index_1.default).post("/workspaces/999/apikeys");
            expect(response.status).toBe(404);
            expect(response.body.error).toBe("Workspace not found");
        });
        it("should return 400 for invalid workspace ID", async () => {
            const response = await (0, supertest_1.default)(index_1.default).post("/workspaces/invalid/apikeys");
            expect(response.status).toBe(400);
    ...
    describe("GET /workspaces/:wsId/stats", () => {
        ...
        it("should get workspace stats successfully", async () => {
            ...
            testHelpers_1.mockQuery.mockImplementation((0, testHelpers_1.createMockImplementation)([
                (query, _params) => {
                    ...
                    if (query.includes("SELECT COUNT(*) FROM tasks WHERE workspace_id = $1 AND due_date < NOW() AND status != 'done'")) {
                        return Promise.resolve({
        ...
        it("should return 401 for missing API key", async () => {
            const response = await (0, supertest_1.default)(index_1.default).get(`/workspaces/${testHelpers_1.mockWorkspaceId}/stats`);
