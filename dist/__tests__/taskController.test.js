...
describe("Task Controller", () => {
    ...
    describe("POST /workspaces/:wsId/tasks", () => {
        it("should create a new task successfully", async () => {
            ...
            testHelpers_1.mockQuery.mockImplementation((0, testHelpers_1.createMockImplementation)([
                (query) => {
    ...
    describe("GET /workspaces/:wsId/tasks", () => {
        ...
        it("should filter tasks by status", async () => {
            ...
            const response = await (0, supertest_1.default)(index_1.default)
                ...
                .set("x-api-key", testHelpers_1.mockApiKey);
            expect(response.status).toBe(200);
            expect(testHelpers_1.mockQuery).toHaveBeenCalledWith(expect.stringContaining("AND tasks.status = $"), expect.arrayContaining([testHelpers_1.mockWorkspaceId, "todo", expect.any(Number)]));
        });
        it("should search tasks by query", async () => {
            testHelpers_1.mockQuery.mockImplementation((0, testHelpers_1.createMockImplementation)([
                (query) => {
                    if (query.includes("GROUP BY tasks.id ORDER BY tasks.id ASC LIMIT")) {
                        return Promise.resolve({
                            rows: [testHelpers_1.mockTask],
                            rowCount: 1,
                        });
            ...
            const response = await (0, supertest_1.default)(index_1.default)
                ...
                .set("x-api-key", testHelpers_1.mockApiKey);
            expect(response.status).toBe(200);
            expect(testHelpers_1.mockQuery).toHaveBeenCalledWith(expect.stringContaining("AND (tasks.title ILIKE $2 OR tasks.description ILIKE $2)"), expect.arrayContaining([testHelpers_1.mockWorkspaceId, "%test%", expect.any(Number)]));
        });
        it("should filter tasks by tag", async () => {
            ...
            const response = await (0, supertest_1.default)(index_1.default)
                ...
                .set("x-api-key", testHelpers_1.mockApiKey);
            expect(response.status).toBe(200);
            expect(testHelpers_1.mockQuery).toHaveBeenCalledWith(expect.stringContaining("AND tasks.id IN"), expect.arrayContaining([testHelpers_1.mockWorkspaceId, "urgent", expect.any(Number)]));
        });
    });
    ...
    describe("DELETE /workspaces/:wsId/tasks/:taskId", () => {
        it("should delete a task successfully", async () => {
            testHelpers_1.mockQuery.mockImplementation((0, testHelpers_1.createMockImplementation)([
                (query) => {
                    if (query.includes("SELECT * FROM tasks WHERE id = $1 AND workspace_id = $2")) {
                        return Promise.resolve({
                            rows: [testHelpers_1.mockTask],
                            rowCount: 1,
                        });
    ...
    describe("POST /workspaces/:wsId/tasks/:taskId/tags/:name", () => {
        it("should attach tag to task successfully", async () => {
            testHelpers_1.mockQuery.mockImplementation((0, testHelpers_1.createMockImplementation)([
                (query) => {
                    ...
                    if (query.includes("INSERT INTO task_tags")) {
                        return Promise.resolve({
                            rows: [{ tag_id: testHelpers_1.mockTag.id, task_id: testHelpers_1.mockTask.id }],
                            rowCount: 1,
                        });
                    }
                    return null;
            ...
            const response = await (0, supertest_1.default)(index_1.default)
                .post(`/workspaces/${testHelpers_1.mockWorkspaceId}/tasks/1/tags/urgent`)
                .set("x-api-key", testHelpers_1.mockApiKey);
            expect(response.status).toBe(201);
