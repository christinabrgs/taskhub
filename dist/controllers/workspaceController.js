...
async function generateAPIKey(_req, res) {
    ...
    try {
        ...
        const workspace = await connection_1.default.query("SELECT * FROM workspaces WHERE id = $1", [
            wsId,
        ]);
...
async function getStatsForWorkspace(_req, res) {
    try {
        ...
        if (!workspace.rows.length)
            return res.status(404).json({ error: "Workspace not found" });
        const countStatus = await connection_1.default.query("SELECT status, COUNT(*) FROM tasks WHERE workspace_id = $1 GROUP BY status", [wsId]);
        const topTags = await connection_1.default.query("SELECT t.name, COUNT(*) as tag_count FROM task_tags tt JOIN tags t ON tt.tag_id = t.id JOIN tasks ta ON tt.task_id = ta.id WHERE ta.workspace_id = $1 GROUP BY t.name ORDER BY tag_count DESC LIMIT 5", [wsId]);
        const overdueTasks = await connection_1.default.query("SELECT COUNT(*) FROM tasks WHERE workspace_id = $1 AND due_date < NOW() AND status != 'done'", [wsId]);
        res
            .status(200)
            .json({
...
workspaceRouter.post("/:wsId/apikeys", validateAPIKey_1.validateWsId, generateAPIKey);
workspaceRouter.get("/:wsId/stats", validateAPIKey_1.validateWsId, validateAPIKey_1.validateApiKey, getStatsForWorkspace);
